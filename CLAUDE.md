...
- ~~Add progress view for short generation/analysis job on project ID page (show which shorts are being processed)~~ ✅ Done
- Scheduling system for YT/TikTok/Instagram

### Refactor: Short Container Architecture (Future)

**Goal:** Decouple short creation from short processing to improve UX and fault tolerance.

**Current flow:**
1. Analysis job runs AI to get suggestions
2. For each suggestion, immediately: extract clip → upload → generate social content → create DB record
3. Shorts only appear in UI once fully processed
4. If job fails mid-way, partial shorts exist but retry creates duplicates (mitigated by time-range check)

**Proposed flow:**
1. Analysis job runs AI to get suggestions
2. Create "short containers" in DB immediately with status `pending` and metadata:
   ```
   {
     id, projectId, transcriptionSlice, startTime, endTime,
     status: 'pending',
     tasks: {
       clip_extraction: 'pending',
       thumbnail_extraction: 'pending',
       social_content: 'pending'  // per platform
     }
   }
   ```
3. Queue individual `short_processing` jobs for each short (linked via `shortId`)
4. Each `short_processing` job:
   - Downloads source video (or uses cached version)
   - Extracts clip → updates `tasks.clip_extraction: 'completed'`
   - Extracts thumbnail → updates `tasks.thumbnail_extraction: 'completed'`
   - Generates social content → updates `tasks.social_content: 'completed'`
   - Sets overall status to `completed` when all tasks done

**Benefits:**
- **Immediate visibility:** Shorts appear in UI as soon as AI suggests them (with "processing" status)
- **Granular retry:** Can retry individual shorts or even individual tasks (e.g., just regenerate social content)
- **Better progress:** UI can show per-short task progress (clip ✓, thumbnail ✓, social ⏳)
- **Parallel processing:** Multiple shorts can process concurrently as separate jobs
- **Fault isolation:** One short failing doesn't affect others
- **Resume capability:** If worker restarts, pending shorts are automatically picked up

**Schema changes needed:**
- Add `tasks` JSONB column to `shorts` table (or create `short_tasks` table)
- Add `short_processing` job type to `processing_jobs`
- Potentially add `analysisJobId` FK to shorts to track which analysis created them

**Migration path:**
1. Add new columns/tables with defaults
2. Update processor to create containers first, then queue individual jobs
3. Add new `short_processing` job handler
4. Update frontend to show pending shorts with task-level progress
5. Keep backward compatibility with existing shorts (no tasks = legacy)
